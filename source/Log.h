...
{
public:
   Log(int32_t logLimit, const char *logFileName, int32_t debugLevel, bool echoToConsole);
   ...
   void  UseCRInsteadOfLF(bool crOverLF) { ib_CROverLF = crOverLF; if (crOverLF) ib_NeedLF = true; };
...
private:
   ...
   void     Write(const char *fileName, const char *logMessage, int32_t toConsole, int32_t respectLimit);
